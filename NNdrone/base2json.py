...
BaseModel -> lwtnn converter

Convert BaseModel to JSON format for Keras with lwtnn.
Similar to https://github.com/lwtnn/lwtnn - converters/keras2json.py

Only considering linear dense models

help:
    $ python converters/base2json.py --help
...
class BASE2JSON(object):
    ...
    def __init__(self):
        ...
        self.model_pkl_file_  = None  # BaseModel pkl file
        ...
        # model
        self.arch_         = "dense"    # BaseModel is fully connected always
        self.activation_   = "sigmoid"  # assume sigmoid activation if none specified
    ...
    def run(self):
        '''Load, convert and save BaseModel to a JSON file'''
        print("BaseModel to lwtnn JSON converter")
    ...
    def _read_model_pkl(self):
        '''Load BaseModel from *.pkl file'''
        print(" - Reading BaseModel from pkl file")

        self.base = joblib.load(open(self.model_pkl_file_, 'rb'))  # load BaseModel

        self.activation_         = 'sigmoid'               # main activation function
        self.n_inputs_           = self.base._n_features   # number of inputs
        self.n_outputs_          = self.base._n_outputs    # number of outputs
        self.n_layers_           = len(self.base._layers)  # number of layers = input + hidden + output
        self.layers_             = self.base._layers       # layers = input + hidden + output
        self.out_activation_     = 'sigmoid'               # output layer activation function
    ...
    def _convert_model(self):
        ...
        for l in range(self.n_layers_):
            ...
            layer["weights"]      = self.layers_[l]['weight'].T.flatten().tolist()
            layer["bias"]         = self.layers_[l]['bias'].flatten().tolist()
...
if __name__ == '__main__':
    ...
    parser_spec.add_argument('-j', '--save-vars-json', action = 'store', dest = 'save_vars_json',
    ...
    if args.subcommand == 'list':
        ...
        conv.var_json_file_ = args.save_vars_json if args.save_vars_json is not None else str(conv.var_spec_file_).split('.')[0] + '_gen.json'
