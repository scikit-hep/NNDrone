...
for etas, phis, ets in zip(sig.charged_eta_centred, sig.charged_phi_centred, sig.charged_et):
    ...
    for x, y, z in zip(etas, phis, ets):
        point = [x, y, z]
        points.append(point)
    charged_et_image = make_image(points, [[-0.4, 0.4],[-0.4, 0.4]], [15,15], value_index = -1, multiplicity = False, reflect_phi_dim = 1)
    charged_multi_image = make_image(points, [[-0.4, 0.4],[-0.4, 0.4]], [15,15], reflect_phi_dim = 1)
...
for etas, phis, ets in zip(sig.neutral_eta_centred, sig.neutral_phi_centred, sig.neutral_et):
    ...
    for x, y, z in zip(etas, phis, ets):
        point = [x, y, z]
        points.append(point)
    neutral_et_image = make_image(points, [[-0.4, 0.4],[-0.4, 0.4]], [15,15], value_index = -1, multiplicity = False, reflect_phi_dim = 1)
    neutral_multi_image = make_image(points, [[-0.4, 0.4],[-0.4, 0.4]], [15,15], reflect_phi_dim = 1)
...
# save output
sig.to_hdf('combined_sig_img.h5', 'table')
bkg.to_hdf('combined_bkg_img.h5', 'table')
