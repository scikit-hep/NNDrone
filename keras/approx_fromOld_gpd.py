...
if not useSaved:
    ...
    labels_train = np.ones(len(sig_train) + len(bkg_train))
    for i in range(len(sig_train), len(sig_train) + len(bkg_train)):
        labels_train[i] = 0.0  # conv nets don't like -1
    labels_test = np.ones(len(sig_test) + len(bkg_test))
    for i in range(len(sig_test), len(sig_train) + len(bkg_test)):
        labels_test[i] = 0.0  # conv nets don't like -1

    setTrain = np.concatenate((sig_train, bkg_train))
    setTest = np.concatenate((sig_test, bkg_test))
    ...
    classifier.add(LocallyConnected1D(filters = 90, kernel_size = 2, activation = 'sigmoid', input_shape = (len(setTrain[0]), 1)))
...
def fname(prefix, base, a, n, t, suff):
    if '.' in suff:
        suff = str(suff).split('.')[1]  # drop trailing dot
